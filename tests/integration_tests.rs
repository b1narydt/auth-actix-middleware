...
//!
//! Tests use a shared Actix test server instance with sequential execution
//! (--test-threads=1). Each test creates its own AuthFetch client instance
//! (fresh client, matching the TS test pattern).
...
/// Shared server base URL. The actual TestServer is leaked (kept alive).
...
/// using the same identity key. Since Actix test servers use random ports,
